...
function RedisDriver(oplog, client, observer) {
  ...
  this.redisObserver.on('message', function(channel, msg) {
    ...
    console.log('json.parse: ' + (+new Date() - stamp));
    self.subscribers.emit(channel, channel, parsed);
  });
...
RedisDriver.prototype.postSubmit = function(cName, docName, opData, snapshot) {
  ...
  var stamp = +new Date();
  var stringy = JSON.stringify(opData);
  console.log('json.stringify: ' + (+new Date() - stamp));
  this.redis.publish(this._prefixChannel(cName), stringy);
...
RedisDriver.prototype._redisSubmitScript = function(cName, docName, opData, docVersion, callback) {
  if (this.sdc) this.sdc.increment('livedb.redis.submit');
    var stamp = +new Date();
    var string1= JSON.stringify(logEntryForData(opData)); // oplog entry
    var string2 = JSON.stringify(opData); // publish entry
    console.log('json.stringify: ' + (+new Date() - stamp));
  this.redis.eval(
    submitCode,
    // num keys
...
RedisDriver.prototype.cleanupRedisKey = function (cName, docName, callback) {
    this.redis.del(getVersionKey(cName, docName), callback);
};
