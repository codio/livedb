...
Livedb.prototype._trySubmit = function(submitData) {
  ...
  this._lazyFetch(submitData.cName, submitData.docName, function(err, snapshot) {
    ...
    self.driver.getOps(submitData.cName, submitData.docName, from, to, function(err, ops) {
      ...
      if (submitData.expectTransform === 1 &&
          ops.length === 0 &&
          self.driver.cleanupRedisKey) {
        self.driver.cleanupRedisKey(submitData.cName, submitData.docName, function(err) {
          submitData.expectTransform = 2;
      ...
      if (submitData.expectTransform === 2 && ops.length === 0) {
        console.warn("ERROR: CORRUPT DATA DETECTED in document "
                     + submitData.cName + '.' + submitData.docName,
                     submitData, from, to);
        console.warn("ERROR: Unable to recover");
      ...
      self.driver.atomicSubmit(submitData.cName, submitData.docName, opData, submitData.submitOptions, function(err) {
        if (err === 'Transform needed') {
          ...
          submitData.expectTransform = 1;
          console.warn("WARN expectTransform set to true", submitData, opData)
