      ...
      self.driver.getOps(cName, docName, from, null, function(err, ops) {
        ...
        self.driver.atomicSubmit(cName, docName, opData, options, function(err) {
          if (err === 'Transform needed') {
            // When we can't fix the problem with one transform
            // it's very likely we have some version wrong, so
            // we try cleaning up the redis key, and start again.
            if (repetition === 1) {
              self.driver.cleanupRedisKey(cName, docName);
              retry(2);
